...
export const ProductFlow = ({
  ...
  return (
    <div>
      <Title2>{t('flowTitle')}</Title2>
      <Description className="mb-2 text-sm">{t('flowDesc')}</Description>
      <WorldMap
        initialViewState={{
...
const FlowLegend = () => {
  ...
  const items: { Icon: LucideIcon; label: string }[] = [
    { Icon: Factory, label: tc('manufacturer') },
    { Icon: Warehouse, label: tc('distributor') },
    { Icon: Store, label: tc('retailer') },
  ]
  return (
    <div className="p-2 space-y-1 text-xs border border-white rounded-lg shadow-lg bg-white/70 backdrop-blur shadow-black/10">
      <div className="font-semibold">{t('legend')}</div>
